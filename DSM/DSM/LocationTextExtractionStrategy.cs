...
namespace DSM
{
    public class LocationTextExtractionStrategyWithPosition : LocationTextExtractionStrategy
    {
        ...
        public IList<TextLocation> GetLocations(string searchtext)
        {
            ...
            if (locationalResult.Count < 1000)
            {

                for (int i = 0; i < filteredTextChunks.Count; i++)
                //foreach (var chunk in filteredTextChunks)
                {

                    if (filteredTextChunks[i].Text.Trim().Contains(searchtext.Trim()))
                    {
                        //initial
                        textLocations.Add(new TextLocation
                        {
                            Text = filteredTextChunks[i].Text,

                            X = filteredTextChunks[i].Location.StartLocation[0],
                            Y = filteredTextChunks[i].Location.StartLocation[1],
                            X1 = filteredTextChunks[i].Location.EndLocation[0],
                            Y1 = filteredTextChunks[i].Location.EndLocation[1]
                        });
